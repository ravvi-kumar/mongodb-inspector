...
func (s *RelationshipStore) Create(ctx context.Context, r *domain.Relationship) error {
	...
	err := s.db.QueryRowContext(ctx,
		`INSERT INTO relationships (connection_id, source_collection, source_field, target_collection, target_field, confidence, matched_values, sampled_values, status)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		 RETURNING id, created_at, updated_at`,
		r.ConnectionID, r.SourceCollection, r.SourceField, r.TargetCollection, r.TargetField,
		r.Confidence, r.MatchedValues, r.SampledValues, string(r.Status),
...
func (s *RelationshipStore) Get(ctx context.Context, id string) (*domain.Relationship, error) {
	...
	var status string
	err := s.db.QueryRowContext(ctx,
		`SELECT id, connection_id, source_collection, source_field, target_collection, target_field, confidence, matched_values, sampled_values, status, created_at, updated_at
		 FROM relationships WHERE id = $1`, id,
	).Scan(&r.ID, &r.ConnectionID, &r.SourceCollection, &r.SourceField, &r.TargetCollection, &r.TargetField, &r.Confidence, &r.MatchedValues, &r.SampledValues, &status, &r.CreatedAt, &r.UpdatedAt)
	if err != nil {
...
func (s *RelationshipStore) List(ctx context.Context, connectionID string, statusFilter *string) ([]domain.Relationship, error) {
	query := `SELECT id, connection_id, source_collection, source_field, target_collection, target_field, confidence, matched_values, sampled_values, status, created_at, updated_at
	...
	for rows.Next() {
		var r domain.Relationship
		var status string
		if err := rows.Scan(&r.ID, &r.ConnectionID, &r.SourceCollection, &r.SourceField, &r.TargetCollection, &r.TargetField, &r.Confidence, &r.MatchedValues, &r.SampledValues, &status, &r.CreatedAt, &r.UpdatedAt); err != nil {
...
func (s *RelationshipStore) UpdateStatus(ctx context.Context, id string, status domain.RelationshipStatus) (*domain.Relationship, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`UPDATE relationships SET status = $2, updated_at = now()
		 WHERE id = $1
		 RETURNING id, connection_id, source_collection, source_field, target_collection, target_field, confidence, matched_values, sampled_values, status, created_at, updated_at`,
		id, string(status),
	).Scan(&r.ID, &r.ConnectionID, &r.SourceCollection, &r.SourceField, &r.TargetCollection, &r.TargetField, &r.Confidence, &r.MatchedValues, &r.SampledValues, &s2, &r.CreatedAt, &r.UpdatedAt)
	if err != nil {
